...
async def process_oauth_login(
    ...
    if not email:
        raise HTTPException(
            ...
            detail=f"Email not provided by {provider.capitalize()} account",
        )
...
async def google_authentication(
    ...
    return await process_oauth_login("google", payload, background_tasks, db)


@router.post("/facebook", response_model=TokenSchema)
async def facebook_authentication(
    payload: OAuthSchema,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
):
    return await process_oauth_login("facebook", payload, background_tasks, db)


@router.post("/github", response_model=TokenSchema)
async def github_authentication(
    payload: OAuthSchema,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
):
    return await process_oauth_login("github", payload, background_tasks, db)
